...
CALORIE_PROMPT = """Проанализируй это изображение еды и предоставь ТОЛЬКО итоговую калорийность и БЖУ в следующем формате:
...
Будь максимально точным в оценке общей калорийности и содержания БЖУ для ВСЕЙ еды на фото."""
...
async def handle_photo(message: Message):
    ...
    try:
        ...
        if response.text:
            await message.answer(response.text) # Теперь Gemini должен возвращать только итог
